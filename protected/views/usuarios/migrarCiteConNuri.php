...
$dataReader=Usuarios::getCiteConNuri(); 
//print_r($dataReader);
$i=0;
...
echo "<br> ---> ".$i."---> ".$doc['codigo'];
			...
			if ($documentos->insert()) {
				...
				$cite=$doc['codigo']; 
				$row_hoja=Usuarios::getHojasRutaSiacoCite($cite);


				//echo "----> ".print_r($row_hoja);
				//exit();

				$hojasruta= new HojasRuta();
				$hojasruta->isNewRecord = true;
				$hojasruta->nur=$row_hoja['nur'];
				$hojasruta->codigo=$row_hoja['codigo'];
				$hojasruta->nro=0;
				...
				$hojasruta->fk_usuario=$user['id_usuario'];;
				$hojasruta->gestion='2019';
				$hojasruta->activo=1;
